...
DEFAULT_PLAYER_DIR = Path(__file__).resolve().parent / "players"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Treasure Caravan simulator.")
...
def read_player_header(path: Path) -> PlayerHeader:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if {"PLAYER_NAME", "VERSION", "PLAY_TIMES", "WIN", "POINT"}.issubset(values):
            break
    missing = {"PLAYER_NAME", "VERSION", "PLAY_TIMES", "WIN", "POINT"} - set(values)
    if missing:
...
def update_player_header(path: Path, now_text: str, result: str, point_delta: int) -> None:
    ...
    updated = re.sub(
        r"^(FIRST_GAME_DATE|LAST_GAME_DATE|PLAY_TIMES|WIN|POINT)\s*=\s*(.+)$",
        replace_line,
        text,
        flags=re.MULTILINE,
    )
...
def write_result_file(result: GameResult, output_dir: Path) -> Path:
    ...
    path = output_dir / f"treasure_caravan_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    data = {
        ...
        "results": [
            {
                ...
                "banked": player_result.banked,
                "cargo": player_result.cargo,
                "busts": player_result.busts,
                "points": player_result.banked,
            }
...
def append_game_log(now: datetime, result: GameResult) -> None:
    ...
    points = " vs. ".join(f"{name} {player_result.banked}" for name, player_result in zip(result.players, result.results))
    winner = result.winner_name or "tie"
    log_path.open("a", encoding="utf-8").write(f"{now.strftime('%Y%m%d_%H%M%S')} treasure_caravan winner: {winner} {points}\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_game(tuple(ports), seed=seed, max_actions=args.max_actions)
    ...
    print(f"winner: {result.winner_name or 'tie'}")
